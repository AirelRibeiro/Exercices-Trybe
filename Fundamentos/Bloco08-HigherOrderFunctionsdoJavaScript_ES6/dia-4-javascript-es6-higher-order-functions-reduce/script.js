...
const sumPairs = () => {
  return numbers.filter((number) => number % 2 === 0).reduce(sum);
}
...
const comparaNumber = (number1, number2) => (number1 > number2) ? number1 : number2;

const comparaNota = () => {
  return estudantes.map((estudante) => {
    return {
      nome: estudante.nome,
      materia: estudante.materias.reduce((materiaA, materiaB) => (materiaA.nota > materiaB.nota) ? materiaA : materiaB).name,
    }
  });
}

console.log(comparaNota());
