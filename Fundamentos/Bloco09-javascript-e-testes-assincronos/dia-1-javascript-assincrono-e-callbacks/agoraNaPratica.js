...
// Será A, depois B, depois C, visto que não há uma função que determine o tempo, elas serão executadas de forma síncrona
...
// Serão A, C, B; como B e C estão com setTimeout, com o tempo de execução de C sendo menor que o de B, ela será impressa primeiro.
...
const sendMarsTemperature = (callbackOkay, callbackError) => {
  const possible = Math.random() <= 0.6;
  setTimeout(() => possible ? callbackOkay(getMarsTemperature()) : callbackError('Estou muito ocupado'), messageDelay());
};
